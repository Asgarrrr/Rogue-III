import type { WorldSnapshot, SerializedEntity } from "./serialization";
...
export function addFieldMigration(
  ...
  return createEntityMigration(
    ...
    description ?? `Add ${componentName}.${fieldName} with default ${defaultValue}`,
  );
}
  ...
  return createEntityMigration(
    ...
    (entity) => {
      ...
      if (component && oldFieldName in component) {
        const { [oldFieldName]: value, ...rest } = component;
        return {
          ...entity,
          components: {
            ...entity.components,
...
export function renameComponentMigration(
  ...
  return createEntityMigration(
    ...
    toVersion,
    (entity) => {
      if (oldName in entity.components) {
        const { [oldName]: data, ...rest } = entity.components;
        return {
          ...entity,
          components: {
            ...rest,
