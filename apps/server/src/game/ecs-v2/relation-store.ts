...
import type { RelationType, RelationData, StoredRelation } from "./relation";
import { entityIndex } from "./types";

/**
 * Key for relation data storage.
 */
function makeDataKey(
  relationIndex: number,
  source: Entity,
  target: Entity,
): string {
  return `${relationIndex}:${source}:${target}`;
}
...
export class RelationStore {
  ...
  /**
   * Relation data storage: "relationIndex:source:target" → data
   */
  ...
  add<T>(
    ...
    // Track entities with relations
    this.entitiesWithRelations.add(source);
    this.entitiesWithRelations.add(target);

    // Store data if provided
    if (data !== undefined) {
      const key = makeDataKey(relationIndex, source, target);
  ...
  getData<T>(
    ...
    const key = makeDataKey(relation.id.index, source, target);
    ...
    for (const [relationIndex, outgoingMap] of this.outgoing) {
      ...
      if (targetSet) {
        for (const target of targetSet) {
          ...
          // Remove data
          const key = makeDataKey(relationIndex, entity, target);
          this.data.delete(key);
    ...
    for (const [relationIndex, incomingMap] of this.incoming) {
      ...
      if (sourceSet) {
        ...
        for (const source of sourceSet) {
          ...
          // Remove data
          const key = makeDataKey(relationIndex, source, entity);
          this.data.delete(key);
  ...
  clearByType(relation: RelationType): void {
    ...
    // Count relations being removed
    let removedCount = 0;
    for (const targetSet of outgoingMap.values()) {
      removedCount += targetSet.size;
    }

    // Clear data for this relation type
    ...
    // Note: We don't clean up entitiesWithRelations here as entities
    // might still have other relations. This is a minor memory leak
    // but avoids expensive recalculation.
  }
}
