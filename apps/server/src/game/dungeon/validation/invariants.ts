...
export function validateDungeonInvariants(
  ...
  if (opts.checkGrid && dungeon.terrain) {
    validateTerrainInvariants(dungeon, categories.grid);
  }

  if (opts.checkReachability && dungeon.terrain) {
...
/**
 * Validate terrain-related invariants.
 */
function validateTerrainInvariants(
  dungeon: Dungeon,
  violations: string[],
): void {
  ...
  if (terrain.width !== width) {
    violations.push(
      `Terrain width mismatch: expected ${width}, got ${terrain.width}`,
    );
  }

  if (terrain.height !== height) {
    violations.push(
      `Terrain height mismatch: expected ${height}, got ${terrain.height}`,
    );
  }

  // Check tiles array size
  const expectedSize = width * height;
  if (terrain.tiles.length !== expectedSize) {
    violations.push(
      `Terrain tiles size mismatch: expected ${expectedSize}, got ${terrain.tiles.length}`,
    );
    return;
  }
...
function validateReachabilityInvariants(
  ...
  const terrain = dungeon.terrain;
  if (!terrain || dungeon.rooms.length === 0) return;
  ...
  const isWalkable = (x: number, y: number): boolean => {
    ...
    return dungeon.getTile(x, y) !== 0; // 0 = Wall
  };
