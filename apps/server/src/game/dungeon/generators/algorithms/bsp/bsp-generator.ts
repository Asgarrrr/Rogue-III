...
export class BSPGenerator extends DungeonGenerator {
  ...
  generate(): DungeonImpl {
    ...
    // Generate terrain (Uint8Array)
    const terrain = this.gridToTerrain(grid);

    // Generate entity spawn descriptors
  ...
  async generateAsync(
    ...
    // Phase 5: Generate terrain (Uint8Array)
    const terrain = this.gridToTerrain(grid);
    await this.yield(signal);

    // Phase 6: Generate entity spawn descriptors
    const spawnData = this.generateSpawnData(rooms, connections);
    await this.yield(signal);
   ...
   * This separates layout generation (rooms, corridors) from content population
