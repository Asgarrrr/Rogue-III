...
export class CellularGenerator extends DungeonGenerator {
  ...
  constructor(dungeonConfig: DungeonConfig, seeds: DungeonSeed) {
    ...
    // Calculate dynamic maxCavernSize based on dungeon area
    // Large dungeons can have larger connected caverns
    const dungeonArea = dungeonConfig.width * dungeonConfig.height;
    const dynamicMaxCavernSize = Math.max(
      DEFAULT_CELLULAR_CONFIG.caverns.maxCavernSize,
      Math.floor(dungeonArea * 0.8), // Allow caverns up to 80% of dungeon area
    );
    ...
    this.cellularConfig = this.applyPerformanceProfile(
      {
        ...
        caverns: {
          ...DEFAULT_CELLULAR_CONFIG.caverns,
          maxCavernSize: dynamicMaxCavernSize,
        },
        rooms: {
  ...
  generate(): DungeonImpl {
    ...
    const spawnData = this.generateSpawnData(safeRooms, safeConnections);
  ...
  async generateAsync(
    ...
    const terrain = this.gridToTerrain(grid);
    const spawnData = this.generateSpawnData(rooms, connections);
    const checksum = this.calculateChecksum(
      rooms,
      connections,
      terrain,
   ...
   */
  /**
   * Convert Grid to terrain (Uint8Array) for zero-copy ECS integration
   */
  private gridToTerrain(grid: Grid): DungeonTerrain {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        tiles[index] =
          cell === CellType.WALL ? TerrainTileType.Wall : TerrainTileType.Floor;
      }
    }
  ...
  /**
   * Generate entity spawn descriptors using content generator.
   *
   * Uses config.content parameters if provided, otherwise falls back to defaults.
   * This separates layout generation (caverns, rooms) from content population
   * (enemies, items, traps) while keeping both in the checksum for share codes.
   */
  private generateSpawnData(
    rooms: RoomImpl[],
    connections: ConnectionImpl[],
  ): DungeonSpawnData {
    // Use content config from DungeonConfig if provided
    // Otherwise DungeonContentGenerator will use its own defaults
    const contentGen = new DungeonContentGenerator(
      this.seeds,
      this.config.content ?? {},
    );
    const entities = contentGen.generateContent(rooms, connections);

    // Player spawns in center of first room, or dungeon center if no rooms
    const playerSpawn =
      rooms.length > 0
        ? {
            x: Math.floor(rooms[0].centerX),
            y: Math.floor(rooms[0].centerY),
          }
        : {
  ...
  private calculateChecksum(
    ...
    // Hash terrain dimensions and sample tiles for validation
    const terrainData = `${terrain.width}x${terrain.height}:${terrain.tiles.length}`;
    hash = this.updateHash(hash, terrainData);

    // Sample terrain tiles at key positions for checksum
    const samplePoints = [
      0,
      Math.floor(terrain.tiles.length / 4),
      Math.floor(terrain.tiles.length / 2),
      terrain.tiles.length - 1,
    ];
    for (const idx of samplePoints) {
      if (idx < terrain.tiles.length) {
        hash = this.updateHash(hash, terrain.tiles[idx].toString());
      }
    }

    // Hash spawn data
    const spawnDataStr = `${spawnData.playerSpawn.x},${spawnData.playerSpawn.y}:${spawnData.entities.length}`;
    hash = this.updateHash(hash, spawnDataStr);
    ...
    // Include seeds
    const seedData = `${this.seeds.primary},${this.seeds.layout},${this.seeds.rooms},${this.seeds.details}`;
