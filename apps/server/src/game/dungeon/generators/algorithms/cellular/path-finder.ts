...
export class PathFinder {
  ...
  private createMinimumSpanningTree(
    ...
    const MAX_CONSECUTIVE_FAILURES = rooms.length * 2;
    ...
    while (
      ...
      // Find shortest SUCCESSFUL connection from connected to unconnected rooms
      for (const connectedRoom of rooms.filter((r) => connected.has(r.id))) {
        for (const unconnectedRoom of rooms.filter((r) =>
          ...
          const pairKey = `${connectedRoom.id}-${unconnectedRoom.id}`;
      ...
      // Try to find a path
      const path = this.findPath(bestConnection.from, bestConnection.to, grid);

      if (path.length > 0) {
        // SUCCESS: Path found, create connection
        connections.push(
          new ConnectionImpl(
            bestConnection.from,
            bestConnection.to,
        ...
        // FAILURE: No path found, mark this pair as failed and try next candidate
        const pairKey = `${bestConnection.from.id}-${bestConnection.to.id}`;
        failedPairs.add(pairKey);
        consecutiveFailures++;
      }
    }

    // If there are still unconnected rooms, try aggressive fallback
    if (unconnected.size > 0) {
      const fallbackConnections = this.connectRemainingRoomsAggressively(
        rooms,
        connected,
        unconnected,
        grid,
      );
      connections.push(...fallbackConnections);
    }
  ...
  /**
   * Aggressively connect remaining isolated rooms by trying all possible pairs
   * and using more permissive pathfinding settings
   */
  private connectRemainingRoomsAggressively(
    rooms: RoomImpl[],
    ...
    const connections: ConnectionImpl[] = [];
    const stillUnconnected = new Set(unconnected);

    for (const unconnectedId of unconnected) {
      const unconnectedRoom = rooms.find((r) => r.id === unconnectedId);
      if (!unconnectedRoom) continue;

      // Try connecting to ANY connected room, sorted by distance
      const candidates = rooms
        .filter((r) => connected.has(r.id))
        .map((r) => ({
          room: r,
          distance: this.calculateRoomDistance(unconnectedRoom, r),
        }))
        .sort((a, b) => a.distance - b.distance);

      for (const candidate of candidates) {
        // Use more permissive A* pathfinding (allow tunneling through walls)
        const path = this.findPathAStar(
          { x: unconnectedRoom.centerX, y: unconnectedRoom.centerY },
          { x: candidate.room.centerX, y: candidate.room.centerY },
          grid,
        );
        ...
        if (path.length > 0) {
          ...
          connected.add(unconnectedRoom.id);
          stillUnconnected.delete(unconnectedRoom.id);
          break; // Successfully connected this room
        }
      }
    }
