...
export class Grid {
  ...
  /**
   * Optimized CA step: write results into an existing destination Grid (no allocation).
   * Caller must ensure src !== dst. Returns dst for chaining.
   */
  applyCellularAutomataInto(
    ...
    for (let y = 0; y < height; y++) {
      const yOff = y * width;
      for (let x = 0; x < width; x++) {
        // Manual 8-neighbor count with bounds checks; treat OOB as WALL
        let neighbors = 0;
        const xm1 = x - 1;
        const xp1 = x + 1;
        const ym1 = y - 1;
        ...
        if (ym1 >= 0) {
          ...
          // three out-of-bounds neighbors count as walls
          neighbors += 3;
        }
