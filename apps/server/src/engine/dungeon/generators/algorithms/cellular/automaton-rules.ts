...
export class AutomatonRules {
  ...
  evolveGrid(grid: Grid): Grid {
    // Ping-pong two buffers to avoid allocations inside the loop
    let src = grid;
    let dst = grid.clone();

    for (let i = 0; i < this.config.iterations; i++) {
      src.applyCellularAutomataInto(
        this.config.survivalMin,
        this.config.birthMin,
        dst,
      );
      // Enforce border walls after each iteration to keep edges sealed
      this.addBorderWalls(dst);
      // swap
      const tmp = src;
      src = dst;
      dst = tmp;
  ...
  smoothGrid(grid: Grid): Grid {
    ...
    const dst = grid.clone();
    const srcData = src.getRawData();
    const dstData = dst.getRawData();
    ...
    for (let y = 1; y < height - 1; y++) {
      ...
      for (let x = 1; x < width - 1; x++) {
        ...
        // 8-neighbors inline
        if (srcData[yOffM1 + xm1] === CellType.WALL) neighbors++;
        if (srcData[yOffM1 + x] === CellType.WALL) neighbors++;
        if (srcData[yOffM1 + xp1] === CellType.WALL) neighbors++;
        if (srcData[yOff + xm1] === CellType.WALL) neighbors++;
        if (srcData[yOff + xp1] === CellType.WALL) neighbors++;
        if (srcData[yOffP1 + xm1] === CellType.WALL) neighbors++;
        if (srcData[yOffP1 + x] === CellType.WALL) neighbors++;
        if (srcData[yOffP1 + xp1] === CellType.WALL) neighbors++;

        const current = srcData[yOff + x] as CellType;
        if (current === CellType.WALL && neighbors <= 2)
          dstData[yOff + x] = CellType.FLOOR;
        else if (current === CellType.FLOOR && neighbors >= 6)
          dstData[yOff + x] = CellType.WALL;
        else dstData[yOff + x] = current;
      }
    }
  ...
  private applyMazeRules(grid: Grid): Grid {
    ...
    const result = grid.applyCellularAutomata(
      this.config.survivalMin,
      this.config.birthMin,
    );
  ...
  private openMoreSpace(grid: Grid): Grid {
    ...
    for (let y = 1; y < height - 1; y++) {
      ...
      for (let x = 1; x < width - 1; x++) {
        ...
        if (srcData[yOffP1 + x] === CellType.WALL) neighbors++;
        if (srcData[yOffP1 + xp1] === CellType.WALL) neighbors++;
        const floorNeighbors = 8 - neighbors;
        dstData[idx] = floorNeighbors >= 5 ? CellType.FLOOR : CellType.WALL;
      }
    }
