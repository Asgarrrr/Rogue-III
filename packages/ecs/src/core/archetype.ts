...
export class Archetype {
  ...
  private readonly componentData: (ComponentColumns | null)[] = [];
  ...
  constructor(id: number, componentTypes: ComponentClass[]) {
    ...
    for (const type of componentTypes) {
      ...
      if (!meta.isTag) {
        ...
        this.componentData[meta.id.index] = { meta, columns, fieldIndex };
      }
    }
    this.mask = mask;
  ...
  hasComponent(componentIndex: number): boolean {
    return this.mask.has(componentIndex);
  }

  allocateRow(entity: Entity): number {
    if (this._count >= this._capacity) this.grow();
    ...
    this.componentChangeFlags[row] = this.mask.toBigInt();
    this._count++;
  ...
  freeRow(row: number): Entity | null {
    ...
    if (row !== lastRow) {
      ...
      for (const cc of this.componentData) {
        if (!cc) continue;
  ...
  setComponentData(
    ...
    for (const col of cc.columns) {
      ...
      // Only update fields that are explicitly provided -> preserves existing values for partial updates
      if (value !== undefined) {
        col.data[row] = value;
      }
    }
  ...
  copyComponentDataFrom(
    ...
    const targetCc = this.componentData[componentIndex];
    const sourceCc = source.componentData[componentIndex];
  ...
  getComponentColumns(componentIndex: number): ComponentColumns | undefined {
    return this.componentData[componentIndex] ?? undefined;
  }
  ...
  hasComponentChanged(row: number, componentIndex: number): boolean {
    return (
      (this.componentChangeFlags[row]! & (1n << BigInt(componentIndex))) !== 0n
    );
  }
...
export class ArchetypeGraph {
  ...
  /**
   * Monotonic epoch counter - increments on any structural change.
   * Used by QueryCache for correct invalidation (Bevy-style pattern).
   */
  private _epoch = 0;
  ...
  getOrCreateArchetype(componentTypes: ComponentClass[]): Archetype {
    ...
    if (!archetype) {
      ...
      this._epoch++; // Increment epoch on new archetype creation
    }
  ...
  getMatchingArchetypes(
    withMask: ComponentMask,
    withoutMask: ComponentMask,
  ): Archetype[] {
    const result: Archetype[] = [];
    for (const archetype of this.archetypes) {
      if (
        archetype.mask.containsAll(withMask) &&
        !archetype.mask.intersects(withoutMask)
      ) {
  ...
  private maskToKey(mask: ComponentMask): string {
    return mask.toKey();
  }
}
