...
export class StringPool {
   ...
   *
   * @param force - Must be true to confirm the dangerous operation
   * @throws Error if force is not true or if there are active references
   *
   * WARNING: This invalidates all existing string indices!
   * Only use this when you're certain no components hold string indices.
   */
  clear(force: boolean = false): void {
    if (!force) {
      throw new Error(
        "StringPool.clear() invalidates all string indices. " +
          "Pass force=true to confirm, or use a new StringPool instead.",
      );
    }

    // Check for active references
    const activeRefs = this.refCounts.reduce((sum, count) => sum + count, 0);
    if (activeRefs > 0) {
      throw new Error(
        `Cannot clear StringPool: ${activeRefs} active references exist. ` +
          "Ensure all entities with string fields are despawned first.",
      );
    }
