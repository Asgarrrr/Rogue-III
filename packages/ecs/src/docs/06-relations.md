...
```typescript
import { defineRelation } from "./ecs";
...
```typescript
import { ChildOf, Contains, Targets } from "./ecs";
...
Requêter des entités par leurs relations avec `withRelation()` et `withRelationTo()`.
...
// Entités avec une relation sortante vers une cible spécifique
world.query(Position)
  .withRelation(ChildOf, parent)  // Enfants de parent
  .run(view => { ... });

// Entités avec une relation sortante vers N'IMPORTE QUELLE cible
world.query(Position)
  .withRelation(ChildOf, WILDCARD)  // Toutes les entités avec un parent
  .run(view => { ... });

// Entités qui sont la cible d'une relation
world.query(Position)
  .withRelationTo(ChildOf, parent)  // Enfants de parent (inverse)
  .run(view => { ... });
```
...
```typescript
// Trouver tous les items dans un conteneur
const itemsInChest = world.query(Item)
  .withRelation(Contains, chest)
  .collect();

// Trouver toutes les entités avec un parent
const childEntities = world.query(Position)
  .withRelation(ChildOf, WILDCARD)
  .collect();

// Trouver les ennemis ciblés par une tourelle
const targets = world.query(Enemy)
...
Les relations peuvent avoir des données typées attachées.

```typescript
// Définir une relation avec données
...
console.log(data?.slot); // "mainHand"
...
Une relation exclusive ne permet qu'une seule cible par source.

```typescript
const Targeting = defineRelation("Targeting", { exclusive: true });

world.relate(turret, Targeting, enemy1);
world.relate(turret, Targeting, enemy2);
// → enemy1 automatiquement retiré, seul enemy2 reste

const target = world.getTarget(turret, Targeting);
// → enemy2
```

---

## Comparaison avec Flecs (C)

| Feature | Notre ECS | Flecs |
|---------|-----------|-------|
| Relations bidirectionnelles | ✅ O(1) | ✅ |
| Cascade delete | ✅ | ✅ |
| Query by relation | ✅ withRelation() | ✅ Pair queries |
| Relation data | ✅ | ✅ |
| Exclusive relations | ✅ | ✅ |
| Wildcards | ✅ WILDCARD | ✅ |
| clearByType O(k) | ✅ | ✅ |

Notre implémentation est maintenant à parité avec les features essentielles de Flecs.
