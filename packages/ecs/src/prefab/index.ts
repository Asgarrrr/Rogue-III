...
export class PrefabRegistry {
  ...
  private resolveComponents(
    name: string,
  ): Map<ComponentClass, ComponentInit<unknown> | undefined> {
   ...
   */
  private resolveOnCreate(
    name: string,
  ): Array<(entity: Entity, world: World) => void> {
    const def = this.prefabs.get(name);
    if (!def) return [];
  ...
  spawnMany(
    ...
    overridesFn?: (
      index: number,
    ) => Map<ComponentClass, Partial<ComponentData<unknown>>> | undefined,
  ): Entity[] {
